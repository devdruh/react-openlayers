        ...
        <>
            {
                layerLegendUrl && layerLegendUrl?.map((data) => {
                    ...
                    return data.visible ?
                        <Accordion collapseAll className="border-none rounded-none" data-active-classes="bg-white dark:bg-gray-800 text-gray-900 dark:text-white" data-inactive-classes="text-gray-500 dark:text-gray-400" key={data.id}>
                            <Accordion.Panel key={data.id}>
                                <Accordion.Title className="py-1 border-0 px-3 focus:ring-0 focus:ring-gray-700 border-gray-200 text-sm hover:rounded-none bg-slate-100 hover:bg-slate-300 text-gray-800 border-b border-b-sky-200 dark:border-slate-400 dark:last:border-b-0 first:rounded-b-none first:rounded-t-none dark:bg-gray-800">{data.title}</Accordion.Title>
