                            ...
                            <div className='grid md:grid-cols-2 items-center p-4 sm:p-6 mb-3 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700'>
                                    ...
                                    <div className='flex sm:flex-row justify-center '>
                                        {
                                            currentCondition?.iconCode ?
                                                <img src={`https://meteo.gc.ca/weathericons/${currentCondition?.iconCode}.gif`} className='translate-y-1' width={60} height={51} alt={`${currentCondition?.condition}`} /> : null
                                        }
                                        ...
                                        <div className='text-xs pl-4 whitespace-nowrap'>
                                            <p className='font-normal text-gray-500 dark:text-gray-400'><span className='max-sm:dark:font-medium'>{t('common:dewPoint')}:</span> {currentCondition?.dewpoint}°C </p>
                                            <p className='font-normal text-gray-500 dark:text-gray-400'><span className='max-sm:dark:font-medium'>{t('common:humidity')}:</span> {currentCondition?.relativeHumidity}%</p>
                                            {
                                                currentCondition?.wind ?
                                                    <p className='font-normal text-gray-500 dark:text-gray-400'><span className='max-sm:dark:font-medium'>{t('common:wind')}:</span> {
                                            ...
                                            {
                                                currentCondition?.windChill !== undefined ? <p className='font-normal text-gray-500 dark:text-gray-400'><span className='max-sm:dark:font-medium'>{t('common:windChill')}:</span> {currentCondition?.windChill}</p> : null
                                            }
                                ...
                                <div>
                                    <div className='text-right'>
                                        <p className='text-sm md:text-lg max-sm:font-medium text-gray-500 dark:text-gray-400'>{t('common:weather')}</p>
                                        <p className='text-xs md:text-sm font-thin dark:font-normal text-gray-500 dark:text-gray-400'>{new Date(currentCondition?.dateTime[1].year, currentCondition?.dateTime[1].month - 1, currentCondition?.dateTime[1].day, currentCondition?.dateTime[1].hour, currentCondition?.dateTime[1].minute).toLocaleDateString(i18n.language === 'fr' ? 'fr-CA' : undefined, { weekday: 'short', hour: 'numeric', minute: 'numeric', timeZoneName: 'short' })}</p>
                                        <p className='text-xs md:text-sm font-thin dark:font-normal text-gray-500 dark:text-gray-400'>{currentCondition?.condition}</p>
                                    </div>
                                </div>
                                <div className='col-span-2 columns-2 align-middle'>
                                    <div className='pt-2'><p className='text-xs font-normal text-gray-400 dark:text-gray-400'><span className='max-sm:dark:font-medium'>{t('common:observedAt')}:</span> {currentCondition?.station}</p></div>

                                    <div className='pt-2 max-sm:pt-4 text-right'><p className='flex justify-end text-xs font-normal text-gray-400 dark:text-gray-400'>
